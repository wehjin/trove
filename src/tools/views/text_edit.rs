...
impl TextEditor {
	...
	pub fn with_text(self, text: String) -> Self {
		let new_cursor_index = Self::fit_cursor_to_text(&text, self.cursor_index);
	...
	pub fn with_cursor_index(self, cursor_index: usize) -> Self {
		let new_cursor_index = Self::fit_cursor_to_text(&self.text, cursor_index);
	...
	fn fit_cursor_to_text(text: &String, cursor_index: usize) -> usize {
		let text_len = text.chars().count();
		cursor_index.min(text_len + Self::CURSOR_LEN)
	}
